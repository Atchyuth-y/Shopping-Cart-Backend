...
public class CartServiceImpl implements CartService {
    ...
    public Cart getcartById(int cartId) throws CartException {
        logger.info("Fetching Cart by ID inprogress...");
    ...
    public Cart addCart(Cart cart) {
      ...
      //User user = restTemplate.getForObject("http://localhost:5050/profile/show"+cart.getProfileId(), User.class);

       /* Product product = restTemplate.getForObject("http://localhost:9010/product/view/"+cart.getProductId(), Product.class);

        if(product != null) {
            repository.save(cart);

            restTemplate.put("http://localhost:9010/product/update/" + cart.getProductId(), product);

            return "product with id: "+cart.getProductId();
        }
        else{
